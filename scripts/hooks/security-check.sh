...
if ! command -v gitleaks &> /dev/null; then
    ...
    if git grep -E "AKIA[0-9A-Z]{16}" -- ':!scripts/hooks/security-check.sh' 2>/dev/null; then
    ...
    if git grep -iE "(password|secret|token|api_key|private_key)\s*[:=]\s*['\"][^'\"]{8,}" -- ':!package-lock.json' ':!*.md' ':!scripts/hooks/security-check.sh' 2>/dev/null; then
